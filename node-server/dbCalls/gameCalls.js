	...
	getGame: function (socket, id, search) 
	{
        ...
        MongoClient.connect(config.mongoUrl +'fitbaw', function(err, db) 
        {
            ...
            {            	
				...
				collection.findOne({ _id: o_id}, function(err, game) 
				{
					// If theres an error.	
					if (err) 
					{ 
	...
	searchByTeam: function (socket, teamName) 
	{
        ...
        MongoClient.connect(config.mongoUrl +'fitbaw', function(err, db) 
        {
            ...
            else
            {
				...
				collection.find({$or: [{"homeTeam": { $regex: teamName, $options: 'i' }},{"awayTeam": { $regex: teamName, $options: 'i' }}]}, { _id:0 }).toArray(function(err, games) 
				{
					// If theres an error.	
					if (err) 
					{
	...
	*/ 
	insertGame: function(socket, game)
	{
        ...
        MongoClient.connect(config.mongoUrl +'fitbaw', function(err, db) 
        {
            ...
            {
				db.collection('games').insert(game, function(err, records) 
				{
		        	// If there is an error, send error response.
		            if (err) 
		            {            	
		            ...
		            else
		            {
		            	// Set the message to cloned version of contants message.
					 	...
					 	// Set ID to message.
					 	message._id = records[0]._id;

		            	// Send response to client that initiated request.
		            	socket.emit('game:insertResponse', message);
						socket.broadcast.emit('game:inserted', records);
		            }									           			
				});
			}
			// END if error.
	...
	removeGame: function(socket, id)
	{
        ...
        MongoClient.connect(config.mongoUrl +'fitbaw', function(err, db) 
        {
            ...
            else
            {
				...
				db.collection('games').remove({_id: o_id}, function(err, records) 
				{
		        	// If there is an error, send error response.
		            if (err) 
		            {
