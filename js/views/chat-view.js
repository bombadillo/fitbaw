var ChatView = Backbone.View.extend({
  ...
  render: function() {
    ...
    var numMessages = this.collection.where({
      notification: undefined
    }).length;
    var numNotifications = this.collection.where({
      user: undefined
    }).length;
  ...
  removeNotification: function(e) {
    ...
    el.closest('.notification').slideUp(function() {
      $(this).remove()
    });
  ...
  close: function() {
    // Send request through socket, passing the ID.
    app.nodeSocket.emit('chat:leave', {
      gameRef: this.options.gameRef,
      user: app.userId
    });
    this.remove();
